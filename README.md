...
В локальном режиме (`APP_SEED_DEMO=true`, включён по умолчанию) база
автоматически заполняется демо-данными. Пароль у всех демо-аккаунтов — `admin123`.
...
### Демо-данные vs чистый старт

Что грузится при первом запуске, зависит от переменных окружения:

| Данные | Local (демо) | Production (чисто) |
|---|---|---|
| Схема БД (Flyway) | ✅ | ✅ |
| Справочные теги (17 шт.) | ✅ | ✅ |
| Админ (`ADMIN_EMAIL`/`ADMIN_PASSWORD`) | ✅ | ✅ |
| Демо-выпускники, карточки, вопросы | ✅ | ❌ |

Управляется флагом **`APP_SEED_DEMO`** (`true` — с демо, `false` — чисто).
Демо-данные грузятся один раз (только если в базе ещё нет карточек), повторный
запуск их не дублирует. Начальный админ создаётся из `ADMIN_EMAIL` /
`ADMIN_PASSWORD`, если админа ещё нет.
...
# Начальный администратор (создаётся при первом старте):
ADMIN_EMAIL=you@your-domain
ADMIN_PASSWORD=<надёжный пароль>

# Чистый старт без демо-данных:
APP_SEED_DEMO=false
...
При первом запуске backend применит миграции, создаст справочные теги и одного
администратора из `ADMIN_EMAIL`/`ADMIN_PASSWORD`. Каталог будет пустым —
выпускники появятся по мере реальной регистрации по приглашениям.
